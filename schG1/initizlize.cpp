...
void processgrid(const std::string& gridfname, double** x, double** y, double** z, int* size){
	...
	getline(grid, s);	ss.str(s);	ss>>n;size[0] = n+4;	ss.clear();
	...
	*x = coord1;	ss.clear();
	...
	getline(grid, s);	ss.str(s);	ss>>n;size[1] = n+4;	ss.clear();
	...
	*y = coord2;	ss.clear();
	...
	getline(grid, s);	ss.str(s);	ss>>n;size[2] = n+4;	ss.clear();
...
void heat(int* size, double* hsource){
	...
	for(int ix = 0; ix < nx; ix++){
		for(int iy = 0; iy < ny; iy++){
			for(int iz = 0; iz < nz; iz++){
				double r = (ix-nx/2)*(ix-nx/2) + (iz-nz/2)*(iz-nz/2);
				if(sqrt(r) < 4){
					hsource[iz + iy*nny + ix*nnx] = 1.0e+2;
				}
			}
		}
	}*/
